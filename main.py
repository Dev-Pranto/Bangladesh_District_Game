...
district_list = data['District_Name'].tolist()
xcor = data['X'].tolist()
ycor = data['Y'].tolist()
guessed_district = []
...
while condition:
    ...
    for item in district_list:
        if item == user_input.title():
            index = district_list.index(item)
    ...
    if count > 64 or user_input.title() == 'Exit':
        condition = False
        missed_district = [districts for districts in district_list if districts not in guessed_district]
